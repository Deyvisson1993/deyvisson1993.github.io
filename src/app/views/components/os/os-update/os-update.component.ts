...
export class OsUpdateComponent implements OnInit{
  ...
  findById(): void{
    this.serviceOS.findById(this.os.id).subscribe(resposta =>{
      this.os = resposta;
      this.converteDados();
    })
  }
  ...
  update():void {
    ...
    console.log(this.os)
    this.serviceOS.update(this.os).subscribe(resposta =>{
      this.router.navigate(['os'])
      this.serviceOS.message('OS Atualizada com Sucesso!')
    }, err =>{
      if(err.error.erro.match('Erro na validação dos campos!')){
        this.serviceOS.message('Dados invalidos')
      }
    })
  }
  ...
  // Metodo que sera instanciado na atualização
  converteDados(): void{
